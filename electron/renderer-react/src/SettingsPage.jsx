    ...
    <form id="settings-form" className="settings-layout" onSubmit={form.handleSubmit(submit)}>
      ...
      <section className="settings-card platform-settings">
        ...
        <div className="credential-list">
          ...
          <section className="credential-row"><div className="credential-platform"><strong>TikTok</strong><small>Fresh session for blocked requests</small></div><label className="setting-field credential-wide"><span><strong>Cookies file</strong><small>Leave empty for public content that works without authentication.</small></span><span className="file-input"><input placeholder="TikTok cookies.txt" {...form.register('tiktok_cookies_file')} /><button type="button" className="icon-button" onClick={() => chooseCookies('tiktok_cookies_file')} aria-label="Choose TikTok cookies file"><FolderOpen size={16} /></button></span></label></section>
        </div>
      </section>
    </form>
  </section>
