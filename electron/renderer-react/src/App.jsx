...
import { Activity, Check, Download, FolderOpen, RefreshCw, Settings, Video } from 'lucide-react'
...
export default function App() {
  ...
  const sync = useMutation({ mutationFn: ensureSync, onSuccess: result => { if (result.job_ids?.length || result.job_id) toast.success(`Sync started${result.source_ids?.length ? ` for ${result.source_ids.length} source(s)` : ''}`); queryClient.invalidateQueries() }, onError: error => toast.error(`Auto-sync: ${error.message}`) })
  useEffect(() => {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark"><Video size={18} /></span><div><strong>Reup Studio</strong><small>Internal workspace</small></div></div><nav aria-label="Main navigation">{nav.map(item => { const Icon = item.icon; const count = item.id === 'sources' ? overview?.sources : item.id === 'review' ? overview?.counts?.review : item.id === 'queue' ? overview?.counts?.queued : null; return <button key={item.id} className={`nav-item ${view === item.id ? 'active' : ''}`} onClick={() => setView(item.id)}><Icon size={18} aria-hidden="true" /><span>{item.label}</span>{count != null && <b>{count}</b>}</button> })}</nav><div className="sidebar-footer"><span className={running.length ? 'syncing' : ''} />{running.length ? `${running.length} task${running.length === 1 ? '' : 's'} active` : 'Auto-sync ready'}</div></aside><main><header className="topbar"><p>TEAM REUP / {view.toUpperCase()}</p><div className="topbar-actions"><button className="top-refresh" onClick={() => queryClient.invalidateQueries()}><RefreshCw size={16} /> Refresh</button><button className={`activity-button ${running.length ? 'running' : ''}`} onClick={() => setActivityOpen(true)}><Activity size={17} /><span>Tasks</span>{activeDownloadPercent != null && <b className="task-menu-progress" aria-label={`Download progress ${activeDownloadPercent}%`}>{activeDownloadPercent}%</b>}{running.length > 0 && <i />}</button></div></header><Suspense fallback={<div className="page"><div className="table-state">Loading workspace…</div></div>}><AnimatePresence mode="wait"><motion.div key={view} initial={reducedMotion ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={reducedMotion ? {} : { opacity: 0, y: -4 }} transition={{ duration: .16 }}><Page startJob={() => queryClient.invalidateQueries()} /></motion.div></AnimatePresence></Suspense></main><ActivityDrawer open={activityOpen} onOpenChange={setActivityOpen} /></div>
}
